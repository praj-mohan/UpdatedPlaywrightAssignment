"use strict";
/**
 * Auto-inject fixtures are automatically injected into every step call.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.getBddAutoInjectsFixtures = exports.isBddAutoInjectFixture = void 0;
const BDD_AUTO_INJECT_FIXTURES = {
    $tags: null,
    $test: null,
    $step: null,
    $testInfo: null,
};
function isBddAutoInjectFixture(name) {
    return Object.prototype.hasOwnProperty.call(BDD_AUTO_INJECT_FIXTURES, name);
}
exports.isBddAutoInjectFixture = isBddAutoInjectFixture;
function getBddAutoInjectsFixtures(bddWorld) {
    return {
        $testInfo: bddWorld.testInfo,
        $tags: bddWorld.tags,
        $test: bddWorld.test,
        $step: bddWorld.step,
    };
}
exports.getBddAutoInjectsFixtures = getBddAutoInjectsFixtures;
//# sourceMappingURL=autoInject.js.map